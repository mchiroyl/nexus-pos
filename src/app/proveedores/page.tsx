...
export default function ProveedoresPage() {
  ...
  const [formData, setFormData] = useState({ 
    id: '', documento: '', razon_social: '', direccion: '', telefono: '', email: '', estado: 'Activo' 
  });
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/sheets?range=Proveedores!A:G');
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const values = [
        formData.id || `PROV-${Date.now().toString().slice(-6)}`, 
        formData.documento, formData.razon_social, formData.direccion, 
      ...
      if (editingId) {
        ...
        await fetch('/api/sheets', {
          ...
          body: JSON.stringify({ range: `Proveedores!A${editingId}:G${editingId}`, values })
        });
      } else {
        await fetch('/api/sheets', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ range: 'Proveedores!A:G', values })
        });
      }
      setShowModal(false);
      resetForm();
  ...
  const openEdit = (item: Proveedor) => {
    setFormData({ 
      id: item.id, documento: item.documento || '', razon_social: item.razon_social || '', 
  ...
  const toggleStatus = async (item: Proveedor) => {
    if (!item.rowNumber) return;
    setLoading(true);
    const newStatus = item.estado === 'Activo' ? 'Inactivo' : 'Activo';
    const values = [item.id, item.documento, item.razon_social, item.direccion, item.telefono, item.email, newStatus];
    await fetch('/api/sheets', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ range: `Proveedores!A${item.rowNumber}:G${item.rowNumber}`, values })
    });
    fetchData();
  };

  const resetForm = () => {
    setFormData({ id: '', documento: '', razon_social: '', direccion: '', telefono: '', email: '', estado: 'Activo' });
    setEditingId(null);
  };
    ...
    <main className="p-4 md:p-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Truck className="text-emerald-400" /> Proveedores
        </h1>
        <button 
        ...
        >
          <Plus size={18} /> <span className="hidden md:inline">Nuevo Proveedor</span>
        </button>
      </div>
              ...
              {loading && items.length === 0 ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="p-4 text-white font-medium">{item.razon_social}</td>
