...
export default function MarcasPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/sheets?range=Marcas!A:D');
      if (res.ok) {
    ...
    try {
      if (editingId) {
        ...
        await fetch('/api/sheets', {
          ...
          body: JSON.stringify({ range: `Marcas!A${editingId}:D${editingId}`, values })
        });
      } else {
        const newId = formData.id || `MAR-${Date.now().toString().slice(-6)}`;
        const values = [newId, formData.nombre, formData.descripcion, formData.estado];
        await fetch('/api/sheets', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ range: 'Marcas!A:D', values })
        });
      }
      setShowModal(false);
      resetForm();
  ...
  const toggleStatus = async (item: Marca) => {
    ...
    await fetch('/api/sheets', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ range: `Marcas!A${item.rowNumber}:D${item.rowNumber}`, values })
    });
    fetchData();
  };
    ...
    <main className="p-4 md:p-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Award className="text-emerald-400" /> Marcas
        </h1>
        <button 
        ...
        >
          <Plus size={18} /> <span className="hidden md:inline">Nueva Marca</span>
        </button>
      </div>
