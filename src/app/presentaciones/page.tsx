...
export default function PresentacionesPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/sheets?range=Presentaciones!A:D');
      if (res.ok) {
    ...
    try {
      if (editingId) {
        ...
        await fetch('/api/sheets', {
          ...
          body: JSON.stringify({ range: `Presentaciones!A${editingId}:D${editingId}`, values })
        });
      } else {
        const newId = formData.id || `PRS-${Date.now().toString().slice(-6)}`;
        const values = [newId, formData.nombre, formData.descripcion, formData.estado];
        await fetch('/api/sheets', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ range: 'Presentaciones!A:D', values })
        });
      }
      setShowModal(false);
      resetForm();
  ...
  const openEdit = (item: Presentacion) => {
    setFormData({ id: item.id, nombre: item.nombre, descripcion: item.descripcion, estado: item.estado });
    setEditingId(item.rowNumber || null);
    setShowModal(true);
  };

  const toggleStatus = async (item: Presentacion) => {
    if (!item.rowNumber) return;
    setLoading(true);
    const newStatus = item.estado === 'Activo' ? 'Inactivo' : 'Activo';
    const values = [item.id, item.nombre, item.descripcion, newStatus];
    await fetch('/api/sheets', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ range: `Presentaciones!A${item.rowNumber}:D${item.rowNumber}`, values })
    });
    fetchData();
  };
    ...
    <main className="p-4 md:p-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <PackageSearch className="text-emerald-400" /> Presentaciones
        </h1>
        <button 
        ...
        >
          <Plus size={18} /> <span className="hidden md:inline">Nueva Presentación</span>
        </button>
      </div>
              ...
              {loading && items.length === 0 ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-800/50 transition-colors">
                    <td className="p-4 font-medium text-gray-300">{item.id}</td>
                    <td className="p-4 text-white font-medium">{item.nombre}</td>
                    <td className="p-4 text-gray-400 hidden md:table-cell">{item.descripcion}</td>
                    <td className="p-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="Ej. Caja x 12, Unidad, Litro"
                  />
                </div>
                <div>
