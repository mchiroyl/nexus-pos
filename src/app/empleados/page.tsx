...
export default function EmpleadosPage() {
  ...
  const [formData, setFormData] = useState({ 
    id: '', nombres: '', apellidos: '', documento: '', telefono: '', estado: 'Activo' 
  });
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/sheets?range=Empleados!A:F');
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const values = [
        formData.id || `EMP-${Date.now().toString().slice(-4)}`, 
        formData.nombres, formData.apellidos, formData.documento, formData.telefono, formData.estado
      ];
      ...
      if (editingId) {
        ...
        await fetch('/api/sheets', {
          ...
          body: JSON.stringify({ range: `Empleados!A${editingId}:F${editingId}`, values })
        });
      } else {
        await fetch('/api/sheets', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ range: 'Empleados!A:F', values })
        });
      }
      setShowModal(false);
      resetForm();
  ...
  const openEdit = (item: Empleado) => {
    setFormData({ 
      id: item.id, nombres: item.nombres || '', apellidos: item.apellidos || '', 
      documento: item.documento || '', telefono: item.telefono || '', estado: item.estado 
    });
    setEditingId(item.rowNumber || null);
    setShowModal(true);
  };

  const toggleStatus = async (item: Empleado) => {
    if (!item.rowNumber) return;
    setLoading(true);
    const newStatus = item.estado === 'Activo' ? 'Inactivo' : 'Activo';
    const values = [item.id, item.nombres, item.apellidos, item.documento, item.telefono, newStatus];
    await fetch('/api/sheets', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ range: `Empleados!A${item.rowNumber}:F${item.rowNumber}`, values })
    });
    fetchData();
  };

  const resetForm = () => {
    setFormData({ id: '', nombres: '', apellidos: '', documento: '', telefono: '', estado: 'Activo' });
    setEditingId(null);
  };
    ...
    <main className="p-4 md:p-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Briefcase className="text-emerald-400" /> Empleados
        </h1>
        <button 
        ...
        >
          <Plus size={18} /> <span className="hidden md:inline">Nuevo Empleado</span>
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-gray-950/50 text-gray-400">
              <tr>
                <th className="p-4 font-medium">Documento / DPI</th>
              ...
              {loading && items.length === 0 ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="p-4 text-white font-medium">{item.nombres} {item.apellidos}</td>
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-1">Documento / DPI</label>
                    <input 
                      type="text" 
